...
export default function AdminPlansPage() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      trial_duration: form.trial ? Number(form.trial_duration) : null,
    };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Plans</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan) => (
            <div key={plan.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                <h3 className="font-black text-slate-900">{plan.nom}</h3>
                <span
                  className={`text-[9px] font-bold px-2 py-1 rounded-lg ${
                    plan.statut === "actif" ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"
                  }`}
                >
                  {plan.statut}
                </span>
              ...
              <p className="text-lg font-black text-slate-900 mb-4">
                {plan.prix} {plan.devise} / {plan.duree} {plan.duree_unite}
              </p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-slate-200 text-slate-400 hover:border-red-200 hover:text-red-600 hover:bg-red-50 transition"
                >
                  <Trash2 size={13} />
                </button>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  placeholder="Prix mensuel de référence"
                ...
                {form.trial && (
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        trial_duration: Number(e.target.value),
                      })
                    }
                    placeholder="Jours"
